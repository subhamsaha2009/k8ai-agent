...
from k8ai.config import load_config, apply_config_to_env, is_configured
from mcp.server.fastmcp import FastMCP

# Load config from ~/.k8ai/ if available (so MCP server works after k8ai init)
if is_configured():
    _cfg = load_config()
    if _cfg:
...
def main():
    """Entry point for 'k8ai-mcp' command."""
